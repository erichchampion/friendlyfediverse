...
describe("trimPostsToLimit", () => {
    it("preserves the currently visible posts even if direction contradicts the viewport", () => {
        ...
        const posts = Array.from({ length: 400 }, (_, i) => ({ id: `${i}` } as any));
        const validViewportPosition = { firstVisibleIndex: 0, lastVisibleIndex: 20 };

        // Test dropping from start (what loadMore uses when appending bottom)
        const trimmedFromStart = trimPostsToLimit(posts, "dropFromStart", validViewportPosition);

        // Test dropping from end (what loadNewer uses when prepending top)
        const trimmedFromEnd = trimPostsToLimit(posts, "dropFromEnd", { firstVisibleIndex: 380, lastVisibleIndex: 400 });
